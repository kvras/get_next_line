...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/24 13:47:25 by miguiji           #+#    #+#             */
/*   Updated: 2023/11/24 14:46:21 by miguiji          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	i = 0;
	if (fd < 0 || BUFFER_SIZE < 1 || read(fd, buffer[fd], 0) < 0 || fd >= 10240)
	{
		if (fd >= 0 && fd < 10240 && buffer[fd])
			return (free(buffer[fd]), buffer[fd] = NULL, NULL);
		return (NULL);
	}
	line = read_wrp(&buffer[fd], fd);
	if (!line)
		return (0);
	return (line);
}
